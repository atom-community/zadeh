...
public:
  Napi::Value filter(const Napi::CallbackInfo &info) {
    // NOTE: not used, as it seems slower than using `ZadehNode::filterIndices` and then filter based on the
    // indices on the JavaScript side. Currently, it is disabled and so `set_candidates` doens't store a
    // reference. If you want to use this function, you should call `strArrFilterer.set_candidates` with the
    // second argument set to `true`
    return strArrFilterer.filter(info[0].As<Napi::String>(), info.Env(),
                                 info[1].As<Napi::Number>().Uint32Value(), info[2].As<Napi::Boolean>(),
                                 info[3].As<Napi::Boolean>());
  }

  Napi::Value filterIndices(const Napi::CallbackInfo &info) {
    const auto env = info.Env();
    const auto filter_indices =
        strArrFilterer.filter_indices(info[0].As<Napi::String>(), info[1].As<Napi::Number>().Uint32Value(),
                                      info[2].As<Napi::Boolean>(), info[3].As<Napi::Boolean>());
  ...
  Napi::Value setTreeFiltererCandidates(const Napi::CallbackInfo &info) {
    ...
    treeFilterer.set_candidates(info[0].As<Napi::Array>(), info[1].As<Napi::String>(),
  ...
  Napi::Value filterTree(const Napi::CallbackInfo &info) {
    return treeFilterer.filter(info[0].As<Napi::String>(), info.Env(),
                               info[1].As<Napi::Number>().Uint32Value(), info[2].As<Napi::Boolean>(),
                               info[3].As<Napi::Boolean>());
  }

  Napi::Value filterIndicesTree(const Napi::CallbackInfo &info) {
    return treeFilterer.filter_indices(info[0].As<Napi::String>(), info.Env(),
                                       info[1].As<Napi::Number>().Uint32Value(), info[2].As<Napi::Boolean>(),
                                       info[3].As<Napi::Boolean>());
  }
...
private:
  ...
  TreeFilterer<Napi::Array, Napi::Object, Napi::Number, Napi::Reference<Napi::Array>, Napi::Env>
      treeFilterer{};
};
