...
template <> Napi::Number init(const size_t value, const Napi::Env &env) {
...
template <> Napi::Object get_at(const Napi::Array &candidates, const uint32_t ind) {
  return candidates.Get(ind).As<Napi::Object>();
}
#endif

template <> Napi::Object get_at(const Napi::Array &candidates, const size_t ind) {
  return candidates.Get(ind).As<Napi::Object>();
}
...
template <> void set_at(Napi::Array &candidates, Napi::Object &&value, const size_t iCandidate) {
  candidates.Set(iCandidate, move(value));
}
...
template <> void set_at(Napi::Array &candidates, Napi::Object &&value, const string ind) {
  candidates.Set(ind, move(value));
}

template <> Napi::Reference<Napi::Array> get_ref(const Napi::Array &arr) { return Napi::Persistent(arr); }
...
template <> Napi::Object init(const Napi::Env &env) { return Napi::Object::New(env); }

template <> Napi::Object copy(const Napi::Object &obj, const Napi::Env &env) {
  auto obj_copy = Napi::Object::New(env);
  env.Global().Get("Object").As<Napi::Object>().Get("assign").As<Napi::Function>().Call({obj_copy, obj});
  return obj_copy;
}

template <> Napi::Array copy(const Napi::Array &arr, const Napi::Env &env) {
...
template <> Napi::Array get_at(const Napi::Object &candidates, const string ind) {
  return candidates.Get(ind).As<Napi::Array>();
}

template <> void set_at(Napi::Object &candidates, string &&value, const string index) {
  candidates.Set(index, move(value));
}

template <> void set_at(Napi::Object &candidates, size_t &&value, const string index) {
  candidates.Set(index, move(value));
}
...
template <> void set_at(Napi::Object &candidates, Napi::Array &&value, const string ind) {
  candidates.Set(ind, move(value));
}
...
template <> Napi::Reference<Napi::Object> get_ref(const Napi::Object &obj) { return Napi::Persistent(obj); }
